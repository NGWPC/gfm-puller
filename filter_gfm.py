    ...
    def from_env(cls):
        ...
        for var, var_type in required_vars.items():
            try:
                ...
                raise ValueError(
                    f"Invalid value for {var}: must be {var_type.__name__}"
                )
...
class S3Uploader:
    ...
    def upload_product_files(
        ...
        try:
            ...
            self.logger.error(f"Failed to upload files for {product_id}: {str(e)}")
            raise

    def construct_key(self, product_id: str, date_input: str) -> str:
        """Construct S3 key for product."""
        return posixpath.join(self.key_root, date_input, product_id)
...
class GFMClient:
    ...
    def _authenticate(self):
        ...
        auth_response = httpx.post(
            f"{self.host}/auth/login", headers=headers, json=params
        )
    ...
    def get_products(self, date_range: tuple, aoi_id: str) -> list:
        ...
        params = {"from": date_range[0], "to": date_range[1], "time": "range"}
    ...
    def _create_aoi(self, coordinates: list) -> str:
        ...
        aoi_params = {
            ...
            "geoJSON": {"type": "Polygon", "coordinates": coords_to_use},
        ...
        for attempt in range(max_retries + 1):  # +1 for initial attempt
            try:
                ...
                if attempt < max_retries:
                    ...
                    delay = (2**attempt) * uniform(
                        1, 2
                    )  # Random delay between 1-2, 2-4, 4-8 seconds
                    sleep(delay)
                    self.logger.warning(
                        f"AOI creation attempt {attempt + 1} failed. Retrying after {delay:.1f} seconds. Error: {str(e)}"
                    )
                    continue

        # If we get here, all retries failed
        self.logger.error(f"AOI creation failed after {max_retries + 1} attempts")
    ...
    def download_product(self, product_id: str, aoi_id: str) -> "Product":
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        raise ValueError(
                            f"Failed to get download link after {max_retries} attempts: {str(e)}"
                        )
            ...
            for zip_attempt in range(max_retries):
                try:
                    ...
                    download_path = os.path.join(
                        self.config.paths["temp"], f"{product_id}.zip"
                    )
                    ...
                    for download_attempt in range(max_retries):
                        try:
                            self.logger.info(
                                f"Process {os.getpid()} downloading {product_id} to {download_path} "
                            ...
                            if total_size > 0 and file_size != total_size:
                                raise ValueError(
                                    f"Incomplete download: got {file_size} bytes, expected {total_size}"
                                )

                            self.logger.info(
                                f"Download completed for {product_id}. File size: {file_size}"
                            )
                            break

                        except requests.Timeout as timeout_error:
                            self.logger.error(
                                f"Timeout error for {product_id} (Attempt {download_attempt + 1}): {str(timeout_error)}"
                            )
                            ...
                            self.logger.error(
                                f"Download error for {product_id} (Attempt {download_attempt + 1}): {str(download_error)}"
                            )
                            if os.path.exists(download_path):
                                os.remove(download_path)
                            if download_attempt < max_retries - 1:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Invalid zip file for {product_id} (Attempt {zip_attempt + 1}/{max_retries}): {str(e)}"
                        )
                        ...
                        if zip_attempt < max_retries - 1:
                            ...
                            raise RuntimeError(
                                f"Failed to get valid zip file after {max_retries} attempts"
                            )
                    ...
                    if zip_attempt < max_retries - 1:
                        self.logger.error(
                            f"Retrying entire download for {product_id} due to error: {str(attempt_error)}"
                        )
                        continue
                    raise
            ...
            raise RuntimeError(
                f"Failed to process product {product_id}: {str(e)}"
            ) from e
...
class Product:
    ...
    def __init__(self, id: str, date: datetime, extract_path: str, logger):
        self.id = id
        self.date = date
    ...
    def calculate_flood_ratios(self) -> dict:
        ...
        flood_files = [
            ...
            for f in files
            if "ENSEMBLE_FLOOD" in f and "tif" in f
        ]
        if not flood_files:
        ...
        def log_debug_info(tile_id: str) -> str:
            ...
            debug_info.extend(
                f"  {f}" for f in [f for f in all_files if "flood" in f.lower()]
            )
            debug_info.append("\nFiles with 'reference' in them:")
            debug_info.extend(
                f"  {f}" for f in [f for f in all_files if "reference" in f.lower()]
            )
            debug_info.append("\nFiles with 'obswater' in them:")
            debug_info.extend(
                f"  {f}" for f in [f for f in all_files if "obswater" in f.lower()]
            )
            debug_info.append("\n")
        ...
        for root, _, files in os.walk(self.extract_path):
            for file in files:
                if "ENSEMBLE_FLOOD" in file and "tif" in file:
                    ...
                    if not tile_id:
                        self.logger.warning(
                            f"Could not extract tile ID from filename: {file}"
                        )
                        continue
                    ...
                    try:
                        ...
                        if ref_path:
                            ...
                            ratio = self._calculate_tile_ratio_from_obswater(
                                flood_path, obswater_path
                            )
        ...
        if not ratios and failed_tiles:
            error_messages = "\n".join(
                f"Tile {tile_id}: {error}" for tile_id, error in failed_tiles
            )
            raise ValueError(
                f"Failed to process any tiles successfully. Errors:\n{error_messages}"
            )

        # If some tiles failed but others succeeded, log the failures but return the successful ratios
        if failed_tiles:
            self.logger.warning("Some tiles failed to process:")
            for tile_id, error in failed_tiles:
                self.logger.warning(f"- Tile {tile_id}: {error}")
            self.logger.info(f"Successfully processed {len(ratios)} tiles.")
    ...
    def _calculate_tile_ratio_from_obswater(
        self, flood_path: str, obswater_path: str
    ) -> float:
        """
    ...
    def get_scene_info(self) -> dict:
        ...
        footprint_file = next(
            (
                ...
                if "footprint" in f.lower()
                or (f.startswith("S1") and f.endswith(".geojson"))
            ),
            None,
        )
...
class FlowProcessor:
    ...
    def create_flowfile(
        ...
        Returns:
            Tuple of (DataFrame with feature_id and discharge columns, NWM version number)
        ...
        try:
            ...
            if not feature_ids:
                error_message = (
                    f"No NWM features found in scene polygon for region {region}"
                )
            ...
            if flow_data_result is None:
                error_message = (
                    f"No NWM flow data found for datetime in region {region}"
                )
                logging.warning(error_message)
                raise ValueError(error_message)
    ...
    def get_flow_data(
        self, target_datetime: datetime, region: str = "conus"
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=".nc") as temp_file:
                ...
                with xr.open_dataset(temp_file.name) as ds:
                    ...
                    nwm_version = str(
                        ds.attrs.get("NWM_version_number", "version_unknown")
                    )
    ...
    def _construct_file_pattern(datetime_obj: datetime, region: str) -> str:
        ...
        if region not in region_map:
            raise ValueError(
                f"Invalid region. Must be one of {list(region_map.keys())}"
            )
...
class ProcessingRecord:
    ...
    def update_on_error(
        ...
        error_type: str,
        error_message: str,
        was_uploaded: Optional[bool] = None,
        has_flowfile: Optional[bool] = None,
    ...
    def update_on_success(
        self, flood_ratios: dict, has_flowfile: bool, was_uploaded: bool
    ):
...
class Logger:
    ...
    def _setup_log_directory(self) -> Path:
        ...
        log_dir = Path(
            f"logs/processing-{self.year}_{self.month:02d}_runtime-{self.run_timestamp}"
        )
    ...
    def start_product_processing(
        self, product_id: str, region: str
    ) -> ProcessingRecord:
        record = ProcessingRecord(
            product_id=product_id, start_time=datetime.now(), region=region
        )
        self.processing_records.append(record)
        self.info(f"Starting processing of product: {product_id}")
        return record

    def log_product_success(
        self,
        record: ProcessingRecord,
        flood_ratios: dict,
        has_flowfile: bool,
...
class Controller:
    ...
    def process_monthly_data(self, year: int, month: int):
        ...
        self.logger.info(
            f"Using {num_processes} processes out of {num_cores} available cores"
        )
                ...
                try:
                    ...
                    if self.debug_mode:
                        ...
                        for product_info in products:
                            try:
                                self.handle_product(
                                    product_info["cell_code"], aoi_id, region
                                )
                            except Exception as e:
                                self.logger.error(
                                    f"Failed to process product {product_info['cell_code']}: {str(e)}"
                                )
                                continue
                    else:
    ...
    def handle_product(self, product_id: str, aoi_id: str, region: str):
        ...
        try:
            ...
            try:
                ...
                if (
                    max(flood_ratios.values(), default=0.0)
                    ...
                    try:
                        ...
                        flow_df, nwm_version = flow_processor.create_flowfile(
                            scene_info["polygon"], scene_info["datetime"], region=region
                        )
                    ...
                    try:
                        s3_uploader = S3Uploader(self.config, self.logger)
                        s3_uploader.upload_product_files(
                            product.extract_path, flood_ratios, flowfile_data
                        )
                        was_uploaded = True
                ...
                if errors_occurred:
                    ...
                    self.logger.log_product_success(
                        record, flood_ratios, has_flowfile, was_uploaded
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="GFM Processing to obtain products that might contain flood observations and their associated flowfiles"
    )
    parser.add_argument("--year", type=int, required=True)
    parser.add_argument("--month", type=int, required=True)
    parser.add_argument(
        "--debug", action="store_true", help="Run in debug mode (sequential processing)"
    )
