...
validate_date() {
    ...
    # Additional validation for month range (01-12)
    month=${1%-*}
...
echo "Batch Processing Run: ${start_date} to ${end_date}" > "$output_file"
...
while [ $((current_year * 100 + current_month)) -le "$end_num" ]; do
    ...
    echo -e "\nProcessing $current_date at $(date)" >> "$output_file"
    echo "----------------------------------------" >> "$output_file"
    
    # Run Python script and redirect output to file
    python filter_gfm.py --month "$current_month" --year "$current_year" >> "$output_file" 2>&1
